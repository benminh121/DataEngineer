...
public class AvroProducer {
    ...
    public AvroProducer() {
        ...
        props.put(AbstractKafkaAvroSerDeConfig.SCHEMA_REGISTRY_URL_CONFIG, "https://pkc-ldvr1.asia-southeast1.gcp.confluent.cloud");
    ...
    public void publishRides(List<RideRecord> rides) throws ExecutionException, InterruptedException {
        ...
        for (RideRecord ride : rides) {
            var record = kafkaProducer.send(new ProducerRecord<>("rides_avro", String.valueOf(ride.getVendorId()),
                    ride), (metadata, exception) -> {
                if (exception != null) {
